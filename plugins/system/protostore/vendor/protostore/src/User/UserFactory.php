...
class UserFactory
{
	...
	public static function login(Input $data)
	{
		...
		if (Factory::getUser()->get('guest') != 1)
		{
			...
			$string                 = Text::_('COM_PROTOSTORE_ELM_CART_USER_ALREADY_LOGGED_IN');
		...
		if ($app->login($credentials, $options) === true)
		{
			...
			if ($userid == 0)
			{
				$string                 = Text::_('COM_PROTOSTORE_ELM_CART_USER_NOT_ENABLED');
				$response['message']    = $string;
				$response['error']      = $string;
				$response['status']     = 'ko';
				$response['errorsList'] = [$string];
			...
			else
			{
				...
				$response['message']    = Text::_('COM_PROTOSTORE_ELM_CART_USER_ALERT_SUCCESSFULLY_LOGGED_IN');
		...
		{
			$string = Text::_('COM_PROTOSTORE_ELM_CART_USER_LOGIN_FAILED');
			// Login failed
			$response['message']    = $string;
			$response['error']      = $string;
			$response['status']     = 'ko';
	...
	public static function register(Input $data)
	{
		...
		if (is_null($username))
		{
			$response['error']      = Text::_('COM_PROTOSTORE_ELM_CART_USER_CHOOSE_A_USERNAME_REQUIRED'); // Username required
			$response['status']     = 'ko';
			$response['statuscode'] = 0;
			$response['errorsList'] = [Text::_('COM_PROTOSTORE_ELM_CART_USER_CHOOSE_A_USERNAME_REQUIRED')];
		...
		if (is_null($password))
		{
			$response['error']      = Text::_('COM_PROTOSTORE_ELM_CART_USER_CHOOSE_A_PASSWORD_REQUIRED'); // Password required
			$response['status']     = 'ko';
			$response['statuscode'] = 1;
			$response['errorsList'] = [Text::_('COM_PROTOSTORE_ELM_CART_USER_CHOOSE_A_PASSWORD_REQUIRED')];
		...
		if (is_null($email))
		{
			$response['error']      = Text::_('COM_PROTOSTORE_ELM_CART_USER_YOUR_EMAIL_REQUIRED'); // Email required
			$response['status']     = 'ko';
			$response['statuscode'] = 2;
			$response['errorsList'] = [Text::_('COM_PROTOSTORE_ELM_CART_USER_YOUR_EMAIL_REQUIRED')];
		...
		if ($usersParams->get('allowUserRegistration'))
		{
			...
			if ($useractivation !== 0)
			{
				...
				Factory::getApplication()->enqueueMessage(Text::_('COM_PROTOSTORE_ELM_CART_USER_ALERT_ACCOUNT_SUCCESSFULLY_CREATED'), 'message');
			}
			else
			{
				self::login($data);
			...
			$response['status']  = 'ok';
			$response['message'] = Text::_('COM_PROTOSTORE_ELM_CART_USER_ALERT_ACCOUNT_SUCCESSFULLY_CREATED');

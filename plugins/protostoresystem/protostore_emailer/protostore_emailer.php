...
class plgProtostoresystemProtostore_emailer extends JPlugin
{
	...
	public function onSendProtoStoreEmail($emailtype, $orderId)
	{
		...
		$this->order    = OrderFactory::get($orderId);
		$this->customer = CustomerFactory::get($this->order->customerid);
		...
		if ($emails)
		{
			...
			foreach ($emails as $email)
			{
				...
				{
					// Log everything
					EmaillogFactory::log( implode(',', $emailto), $emailtype, $this->order->customer_id, $orderId);

					OrderFactory::log($orderId, Text::sprintf('COM_PROTOSTORE_ORDER_EMAIL_SENT_LOG', ucfirst($emailtype), implode(',', $emailto), (Factory::getUser()->name ? Factory::getUser()->name : 'Joomla')));
	...
	private function processReplacements($text)
	{
		...
		$text = str_replace('{order_grand_total}', $this->order->order_total_formatted, $text);
		$text = str_replace('{order_subtotal}', $this->order->order_subtotal_formatted, $text);
		if ($this->order->shipping_total)
		{
			$text = str_replace('{order_shipping_total}', $this->order->shipping_total_formatted, $text);
		}

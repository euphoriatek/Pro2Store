...
class plgProtostore_extendedIo extends CMSPlugin implements AdminViewExtended
{
	...
	private function processUpload(Input $data)
	{
		...
		if (!Folder::exists(JPATH_ADMINISTRATOR . '/components/com_protostore/import_files'))
		{
			Folder::create(JPATH_ADMINISTRATOR . '/components/com_protostore/import_files');
		}

		$filename = File::makeSafe($csv['name']);
		$filename = str_replace(' ', '_', $filename);
		$src      = $csv['tmp_name'];
		$dest     = JPATH_ADMINISTRATOR . '/components/com_protostore/import_files/' . $filename;
		...
		if(!Folder::exists(JPATH_ADMINISTRATOR . '/components/com_protostore/import_files/')) {
			Folder::delete(JPATH_ADMINISTRATOR . '/components/com_protostore/import_files/');
		}

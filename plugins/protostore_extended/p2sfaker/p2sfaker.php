...
class plgProtostore_extendedP2sfaker extends CMSPlugin implements AdminViewExtended
{
	...
	private function getResponse(Input $data): bool
	{
		...
		if ($products > 0)
		{
			for ($x = 1; $x <= $products; $x++)
			{
				...
				$this->generateProduct($data->json->getInt('category'));
	...
	private function generateProduct($catid)
	{
		...
		$productInput->json->set('introtext', $faker->text(150));
		$productInput->json->set('fulltext', $faker->text(250));
		$productInput->json->set('catid', $catid);
		...
		Folder::create(JPATH_SITE . '/images/pro2store_faker/');
		...
		$img  = JPATH_SITE . '/images/pro2store_faker/' . $filename;
		$path = 'images/pro2store_faker/' . $filename;
		...
		$productInput->json->set('taxable', random_int(0, 1));
	...
	private function generateCustomer()
	{
		...
		$result = $db->insertObject('#__protostore_customer', $object);
	...
	private function generateOrder()
	{
		...
		$db->insertObject('#__protostore_order', $object);
		...
		$db->updateObject('#__protostore_order', $object, 'id');
	...
	private function getRandomProductJoomlaId()
	{
		...
		$db  = Factory::getDbo();
		$sql = "SELECT `joomla_item_id` FROM #__protostore_product ORDER BY RAND() LIMIT 1";
	...
	private function getRandomCustomerId()
	{
		...
		$db  = Factory::getDbo();
		$sql = "SELECT `id` FROM #__protostore_customer ORDER BY RAND() LIMIT 1";
	...
	private function getRandomCountryId()
	{
		...
		$db  = Factory::getDbo();
		$sql = "SELECT `id` FROM #__protostore_country ORDER BY RAND() LIMIT 1";
	...
	{
		$db  = Factory::getDbo();
		$sql = "SELECT `id` FROM #__protostore_zone ORDER BY RAND() LIMIT 1";

const p2s_products = {
    ...
    methods: {
        ...
        async updateList() {
            const request = await fetch(this.base_url + "index.php?option=com_ajax&plugin=protostore_ajaxhelper&method=post&task=task&type=products.updatelist&format=raw&limit=0", {
                method: 'post'
            });
        ...
        async filter() {
            ...
            const request = await fetch(this.base_url + 'index.php?option=com_ajax&plugin=protostore_ajaxhelper&method=post&task=task&type=products.filter&format=raw&' + URLparams, {method: 'post'});

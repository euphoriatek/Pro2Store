    ...
    methods: {
       ...
       async save(item) {
        ...
        async filter() {
            ...
            const request = await fetch(this.base_url + 'index.php?option=com_ajax&plugin=protostore_ajaxhelper&method=post&task=task&type=shippingrateszone.filter&format=raw&' + URLparams, {method: 'post'});
...
Vue.createApp(p2s_shippingrateszone).mount('#p2s_shippingrateszone')

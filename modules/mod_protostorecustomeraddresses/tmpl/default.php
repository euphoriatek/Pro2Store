...
<div id="<?= $id; ?>">
    ...
    <span uk-tooltip="<?= Text::_('COM_PROTOSTORE_ELM_CART_USER_ADD_NEW_ADDRESS'); ?>">
	...
	<?php require ModuleHelper::getLayoutPath('mod_protostorecustomeraddresses', 'editAddress'); ?>
	<?php require ModuleHelper::getLayoutPath('mod_protostorecustomeraddresses', 'addAddress'); ?>
    ...
    const <?= $id; ?> = {
        ...
        methods: {
            ...
            async submitUpdateAddress() {
                ...
                const request = await fetch("<?= Uri::base(); ?>index.php?option=com_ajax&plugin=protostore_ajaxhelper&method=post&task=task&type=address.save&format=raw", {
                    method: 'POST',
                    mode: 'cors',
                    cache: 'no-cache',
                    credentials: 'same-origin',
                ...
                if (response.success) {
                    ...
                    this.loading = false;
                    UIkit.notification({
                        message: '<span uk-icon=\'icon: check\'></span> <?= Text::_('COM_PROTOSTORE_ELM_CART_USER_ALERT_ADDRESS_SAVED'); ?>',
                        status: 'success',
                        pos: 'top-center'
                    });
                    this.updateCustomerAddresses();
            ...
            async updateZones(country_id) {
                ...
                const request = await fetch("<?= Uri::base(); ?>index.php?option=com_ajax&plugin=protostore_ajaxhelper&method=post&task=task&type=address.getZones&format=raw&" + URLparams);
            ...
            async deleteAddress(uid) {
                ...
                const request = await fetch("<?= Uri::base(); ?>index.php?option=com_ajax&plugin=protostore_ajaxhelper&method=post&task=task&type=address.remove&format=raw&" + URLparams);
